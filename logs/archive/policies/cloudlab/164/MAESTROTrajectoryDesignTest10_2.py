...
OUTPUT_DIR = f'../../../logs/policies/{int(DATA_PAYLOAD_SIZE / 1e6)}_2/{HCSO_METRIC_ALPHA}/trajs/'
...
if __name__ == '__main__':
    ...
    comm_action = 250.0
    comm_state = [500.0, 625.0, 270.0 * (np.pi / 180.0)]

    r_u_ = comm_action
    r_u, r_gn, psi = comm_state
    x_init = tf.constant([[-65.0, -300.0]], dtype=tf.float64)
    x_final = tf.constant([[0.0, 0.0]], dtype=tf.float64)
    x_gn = tf.constant([[-65.0, -622.0]], dtype=tf.float64)
