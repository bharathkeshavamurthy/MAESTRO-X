...
OUTPUT_DIR = f'../../../logs/policies/{int(DATA_PAYLOAD_SIZE / 1e6)}_min_analysis/{HCSO_METRIC_ALPHA}/trajs/'
...
if __name__ == '__main__':
    ...
    comm_action = 250.0
    comm_state = [500.0, 625.0, 3.025110638976908]

    r_u_ = comm_action
    r_u, r_gn, psi = comm_state
    x_init = tf.constant([[500.0, 0.0]], dtype=tf.float64)
    x_final = tf.constant([[-225.09, 124.05]], dtype=tf.float64)
