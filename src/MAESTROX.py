...
a, bw, n_u, n_r, n_w = 1e3, 20e6, 3, int(1e4), 1024
...
th_us = [np.linspace(start=0.0, stop=2 * np.pi, num=int((2 * np.pi * _r) / min_dist) + 1) for _r in r_us]
...
class ServiceNode(object):
    ...
    def receive(self, src_id, cf):
        ...
        if frame_type == 'CTL':
            ...
            raise NotImplementedError(f'[ERROR] {self.id} ServiceNode receive: REQ receive handled in MoM')
        else:
    ...
    def frequency_reuse(self, reqs, x_u, traj_u, gn_id):
    ...
    def channel_wait(self, gn_id):
        ...
        x_u, traj_u = self.current_position, self.trajectory
        for chn, chn_els in ch_map.items():
            ch_wait, reuse = 0.0, 0
            if len(chn_els['serv']) > 0:
                ch_wait = max([requests[_el]['serv_time'] for _el in chn_els['serv']])
                if len(chn_els['waits']) == 0:
                    reuse = self.frequency_reuse(chn_els['serv'], x_u, traj_u, gn_id)
                else:
                    ...
                    while i < n_wait:
                        ...
                        if i == n_wait - 1 and ch_state[-1]:
                            if self.frequency_reuse(fr_peers, x_u, traj_u, gn_id):
                                reuse = 1
                                continue
    ...
    def model(self, x_gn, ch_w_time):
        wps, vels = self.trajectory['p'], self.trajectory['v']
        return self.current_position, wps, vels
    ...
    def decode(x_gn, d_p, d_v, d_lp):
        ...
        return tf.reduce_sum(t), t_p, tf.reduce_sum(tf.multiply(t, pwr_cost(d_v[:-1], n_w))), e_p
    ...
    def forward(f_p, f_v, f_lp):
        ...
        return tf.reduce_sum(t), t_p, tf.reduce_sum(tf.multiply(t, pwr_cost(f_v[:-1], n_w))), e_p
    ...
    def service(self, x_gn, *args_):
        if self.type == 'UAV':
            ...
            serv_time, energy = self.direct(x_gn)
        return serv_time, energy

    def cost_computation_with_piggybacking(self, gn_id):
        x_gn = ground_node_map[gn_id]
        ch, ch_w_time, reuse = self.channel_wait(gn_id)

        if self.type == 'UAV':
            x_u, _wp, _vel = self.model(x_gn, ch_w_time)
            ...
            mp = int(wp.shape[0] / 2)
            s_time, s_nrg = self.service(x_gn, wp[:mp], vel[:mp], wp[mp:], vel[mp:])
        else:
            s_time, s_nrg = self.service(x_gn)

        return s_time, ch_w_time, s_nrg
...
class MessagingMiddleware(object):
    ...
    def conflict_resolution(gn_id):
        ...
        return server_nodes[nd].id, costs[nd]
...
def gn_request():
    ...
    s_node_id, (s_time, w_time, s_energy) = middleware.conflict_resolution(ground_node_id)
...
channel_resources = [Resource(env) for _ in range(n_c)]
...
[server_nodes.append(ServiceNode(server_node_ids[_u], 'UAV', uavs[_u],
                                 tf.Variable([], dtype=tf.float64))) for _u in range(n_u)]

channel_map = {channel_ids[_i]: [] for _i in range(n_c)}
