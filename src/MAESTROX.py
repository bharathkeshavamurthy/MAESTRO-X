...
"""
CONFIGURATIONS: System Model for Numerical Evaluations (Deployment | Communication | Channel | Mobility Power) 
"""
...
class ServiceNode(object):
    ...
    def __init__(self, uuid_, type_, current_position_, trajectory_):
        ...
        self.users: List = []
        self.type: str = type_
        self.cf_seq_num: int = 0
        self.running: bool = True
        self.state_flag: str = 'OFF'
        self.mw_registered: bool = False
        self.trajectory: tf.Variable = trajectory_
        self.current_position: tf.Variable = current_position_
        self.num_trx: int = n_x_u if self.type == 'UAV' else n_x_bs
        self.env_nodes: Dict = {_node: None for _node in serv_nodes}
        self.transceivers: Dict = {_trx: None for _trx in self.num_trx}
    ...
    def send(self, cost_estimate=None) -> None:
        with self.lock:
    ...
    def receive(self, src_id: uuid, cf: ControlFrame) -> None:
...
class MessagingMiddleware(object):
    ...
    def update(self, src_node: ServiceNode, cf: ControlFrame) -> None:
        k = src_node.id
        if self.running:
            with self.lock:
    ...
    def notify(self, dst_nodes: List, src_id: uuid, cf: ControlFrame) -> None:
        if self.running:
            with self.lock:
                [node.receive(src_id, cf) for node in dst_nodes]

    def expunge(self, node: ServiceNode) -> bool:
        if self.running and node.id in serv_nodes:
            with self.lock:
...
class LinkPerformance(object):
    """
    Link Performance: A2G channel model | Rate adaptation | LoS & NLoS throughputs
    """
    ...
    @staticmethod
    def q(df: int, nc: float, x: float) -> float:
        return 1.0 - ncx2.cdf(x, df, nc)

    def f(self, z: float, *args_: Tuple) -> float:
        df, nc, y = args_
        f_z = self.f_z(z)
    ...
    @staticmethod
    def bisect(f: Any, df: int, nc: float, y: float, low: float, high: float) -> float:
        args_ = (df, nc, y)
    ...
    @staticmethod
    def z(gm: float) -> float:
        return np.sqrt(2.0 * ((2.0 ** (gm / bw_)) - 1.0))

    @staticmethod
    def u(gm: float, d: float, los: bool) -> float:
        return ((2.0 ** (gm / bw_)) - 1.0) / (gm_ * 1.0 if los else kp * (d ** -ap if los else -ap_))

    def los_tgpt(self, d: float, phi: float, r_los: tf.Variable) -> None:
    ...
    def nlos_tgpt(self, d: float, r_nlos: tf.Variable) -> None:
    ...
    def ra_tgpt(self, d: float, phi: float, r_los: tf.Variable, r_nlos: tf.Variable) -> None:
        with ThreadPoolExecutor(n_w) as executor:
            executor.submit(self.nlos_tgpt, d, r_nlos)
            executor.submit(self.los_tgpt, d, phi, r_los)

    def avg_tgpt(self, d_s: tf.Variable, phi_s: tf.Variable) -> Tuple:
