...
"""
data_len, arr_rates, num_uavs, num_gns = [1e6, 10e6, 100e6][0], {1e6: 1.67e-2, 10e6: 3.33e-3, 100e6: 5.56e-4}, 2, n
data_len, arr_rates, num_uavs, num_gns = [1e6, 10e6, 100e6][0], {1e6: 1.67e-2, 10e6: 3.33e-3, 100e6: 5.56e-4}, 3, n
data_len, arr_rates, num_uavs, num_gns = [1e6, 10e6, 100e6][1], {1e6: 1.67e-2, 10e6: 3.33e-3, 100e6: 5.56e-4}, 1, n
data_len, arr_rates, num_uavs, num_gns = [1e6, 10e6, 100e6][1], {1e6: 1.67e-2, 10e6: 3.33e-3, 100e6: 5.56e-4}, 2, n
data_len, arr_rates, num_uavs, num_gns = [1e6, 10e6, 100e6][1], {1e6: 1.67e-2, 10e6: 3.33e-3, 100e6: 5.56e-4}, 3, n
data_len, arr_rates, num_uavs, num_gns = [1e6, 10e6, 100e6][2], {1e6: 1.67e-2, 10e6: 3.33e-3, 100e6: 5.56e-4}, 1, n
data_len, arr_rates, num_uavs, num_gns = [1e6, 10e6, 100e6][2], {1e6: 1.67e-2, 10e6: 3.33e-3, 100e6: 5.56e-4}, 2, n
data_len, arr_rates, num_uavs, num_gns = [1e6, 10e6, 100e6][2], {1e6: 1.67e-2, 10e6: 3.33e-3, 100e6: 5.56e-4}, 3, n
pc, k_1, k_2, z_1, z_2, conf, tol, cvx_conf, cvx_tol = 90.0, 1.0, np.log(100) / 90.0, 9.61, 0.16, 3, 1e-3, 10, 10.0
"""

"""
Configurations-III: BS and UAV(s) Deployment | GNs Deployment (Randomized Generation)
"""
...
"""
UAV Energy Computation Routine | UAV Mobility Power Computation Routine
"""
...
def solve(u, qi, qf, *args):
    ...
    prb.solve('SCS', max_iters=int(1e6), eps_abs=1e-6, eps_rel=1e-6)
...
def sca():
    ...
    for u in range(num_uavs):
        ...
        while c < cvx_conf or not converged(f_prev, f_next):
            ...
            d = tf.sqrt(tf.add(tf.square(h_[u]),
                               tf.square(dxy)))
            phi = tf.asin(tf.divide(h_[u], d))
            lpf.average_throughputs(r_[u], d, phi)
