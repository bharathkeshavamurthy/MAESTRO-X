...
the SCA algorithm determines the UAV's "optimal" trajectory segments (and their corresponding forward velocities).

This script evaluates the convergence performance of this "path planning" for benchmarking w.r.t PSO, CSO, and HCSO.
...
max_iters, eps_abs, eps_rel, warm_start, verbose, sca_conf, sca_tol = int(1e6), 1e-6, 1e-6, True, True, 5, 1e-5
snr_0, k_1, k_2, z_1, z_2, ra_conf, ra_tol = linear((5e6 * 40) / bw_), 1.0, np.log(100) / 90.0, 9.61, 0.16, 10, 1e-10
...
class LinkPerformance(object):
    ...
    def __nlos_throughput(self, d, r_nlos):
        ...
        z_star = self.__bisect(self.__f, df, nc, y, 0, self.__z(
            bw_ * np.log2(1 + (rayleigh.ppf(0.9999999999) ** 2) * snr_0 * kappa * (d ** -a_nlos))))
...
def solve(p_, v_, n_w):
    ...
    with ThreadPoolExecutor(max_workers=n_w) as executor:
        for _i, (_d_gu_, _phi_gu_) in enumerate(zip(d_gu_, phi_gu_)):
            executor.submit(lperf.adapted_throughput, _d_gu_, _phi_gu_, r_los_gu_[_i], r_nlos_gu_[_i], n_w)
    ...
    with ThreadPoolExecutor(max_workers=n_w) as executor:
        for _i, (_d_ub_, _phi_ub_) in enumerate(zip(d_ub_, phi_ub_)):
            executor.submit(lperf.adapted_throughput, _d_ub_, _phi_ub_, r_los_ub_[_i], r_nlos_ub_[_i], n_w)
    ...
    with ThreadPoolExecutor(max_workers=n_w) as executor:
        for _i, (_d_gu, _phi_gu) in enumerate(zip(d_gu, phi_gu)):
            executor.submit(lperf.adapted_throughput, _d_gu, _phi_gu, r_los_gu[_i], r_nlos_gu[_i], n_w)
    ...
    with ThreadPoolExecutor(max_workers=n_w) as executor:
        for _i, (_d_ub, _phi_ub) in enumerate(zip(d_ub, phi_ub)):
            executor.submit(lperf.adapted_throughput, _d_ub, _phi_ub, r_los_ub[_i], r_nlos_ub[_i], n_w)
    ...
    d_ct = cp.sum(s_ + cp.multiply(np.sqrt(s_) * 2.0,
                                   cp.sqrt(s) - np.sqrt(s_)), axis=0)

    s_ct = cp.sum(y_ + cp.multiply(np.sqrt(y_) * 2.0, cp.sqrt(y) - np.sqrt(y_)) +
                  v_ / (v0 ** 2) + cp.multiply(2 * v_ / (v0 ** 2), v - v_), axis=0)

    pwr_cost = cp.transpose(p3 * cp.power(v_var, 3) + p2 * cp.sqrt(y) +
                            p1 * (1 + (3 * cp.power(v_var, 2)) / (utip ** 2)))
    ...
    prb.solve('SCS', max_iters=max_iters, eps_abs=eps_abs, eps_rel=eps_rel, warm_start=warm_start, verbose=warm_start)
