"""
This script evaluates the convergence performance of the Learning Competitive Swarm Optimization (LCSO) algorithm for
UAV trajectory design -- employing the same deployment, communication, channel, and power models as those used in our
proposed MAESTRO framework. This LCSO algorithm is adapted from the following state-of-the-art reference paper.

Reference Paper:
                Borowska B. (2022). Learning Competitive Swarm Optimization.
                Entropy (Basel, Switzerland), 24(2), 283. https://doi.org/10.3390/e24020283.
...
os.environ['TF_XLA_FLAGS'] = '--tf_xla_auto_jit=2 --tf_xla_cpu_global_jit ' \
                             '/home/bkeshav1/workspace/repos/Odin/src/uav-mobility/soa-evaluations/LCSOConvergence.py'
...
def tournament(p_, v_, u_, w_, f_hats_, n_w, save_winners=False, p_w_=None, v_w_=None, u_w_=None, w_w_=None):
    cn, sz = 0, int(p_.shape[0] / 3)
    t = tf.random.shuffle([_ for _ in range(sz)])

    for cn in range(sz):
        for j in range(0, sz * 3, 3):
            with ThreadPoolExecutor(max_workers=n_w) as executor:
                [executor.submit(comm_cost, p_[t[j + h]], v_[t[j + h]], f_hats_[t[j + h]], n_w) for h in range(3)]

            r_vals = tf.random.uniform(shape=[3, ], dtype=tf.float64)
            podium = sorted([t[j], t[j + 1], t[j + 2]], key=lambda x: f_hats_[x], reverse=True)

            if save_winners:
                tf.compat.v1.assign(p_w_[cn], p_[podium[0]], validate_shape=True, use_locking=True)
                tf.compat.v1.assign(v_w_[cn], v_[podium[0]], validate_shape=True, use_locking=True)
                tf.compat.v1.assign(u_w_[cn], u_[podium[0]], validate_shape=True, use_locking=True)
                tf.compat.v1.assign(w_w_[cn], w_[podium[0]], validate_shape=True, use_locking=True)

            u_runner = tf.add(tf.multiply(r_vals[0], u_[podium[1]]),
                              tf.multiply(r_vals[1], tf.subtract(p_[podium[0]], p_[podium[1]])))
            p_runner = tf.add(p_[podium[1]], u_runner)
            tf.compat.v1.assign(p_[podium[1]], p_runner, validate_shape=True, use_locking=True)
            tf.compat.v1.assign(u_[podium[1]], u_runner, validate_shape=True, use_locking=True)

            w_runner = tf.add(tf.multiply(r_vals[0], w_[podium[1]]),
                              tf.multiply(r_vals[1], tf.subtract(v_[podium[0]], v_[podium[1]])))
            v_runner = tf.add(v_[podium[1]], w_runner)
            tf.compat.v1.assign(v_[podium[1]], v_runner, validate_shape=True, use_locking=True)
            tf.compat.v1.assign(w_[podium[1]], w_runner, validate_shape=True, use_locking=True)

            u_loser = tf.add_n([tf.multiply(r_vals[0], u_[podium[2]]),
                                tf.multiply(r_vals[1], tf.subtract(p_[podium[0]], p_[podium[2]])),
                                tf.multiply(r_vals[2], tf.subtract(p_[podium[1]], p_[podium[2]]))])
            p_loser = tf.add(p_[podium[2]], u_loser)
            tf.compat.v1.assign(p_[podium[2]], p_loser, validate_shape=True, use_locking=True)
            tf.compat.v1.assign(u_[podium[2]], u_loser, validate_shape=True, use_locking=True)

            w_loser = tf.add_n([tf.multiply(r_vals[0], w_[podium[2]]),
                                tf.multiply(r_vals[1], tf.subtract(v_[podium[0]], v_[podium[2]])),
                                tf.multiply(r_vals[2], tf.subtract(v_[podium[1]], v_[podium[2]]))])
            v_loser = tf.add(v_[podium[2]], w_loser)
            tf.compat.v1.assign(v_[podium[2]], v_loser, validate_shape=True, use_locking=True)
            tf.compat.v1.assign(w_[podium[2]], w_loser, validate_shape=True, use_locking=True)


def lcso(p, v, u, w, n_w):
    st_idx, end_idx, indices = 0, n_div[0], [_ for _ in range(n)]
    v_ws = tf.Variable(tf.zeros(shape=[len(n_div.keys()), max(n_div.values()),
                                       m_post], dtype=tf.float64), dtype=tf.float64)
    w_ws = tf.Variable(tf.zeros(shape=[len(n_div.keys()), max(n_div.values()),
                                       m_post], dtype=tf.float64), dtype=tf.float64)
    p_ws = tf.Variable(tf.zeros(shape=[len(n_div.keys()), max(n_div.values()),
                                       m_post, 2], dtype=tf.float64), dtype=tf.float64)
    u_ws = tf.Variable(tf.zeros(shape=[len(n_div.keys()), max(n_div.values()),
                                       m_post, 2], dtype=tf.float64), dtype=tf.float64)
    f_hats = tf.Variable(tf.zeros(shape=[len(n_div.keys()), max(n_div.values())], dtype=tf.float64), dtype=tf.float64)

    for k in range(k_max):
        with ThreadPoolExecutor(max_workers=n_w) as executor:
            for i, num in n_div.items():
                executor.submit(tournament, p[st_idx:end_idx], v[st_idx:end_idx], u[st_idx:end_idx],
                                w[st_idx:end_idx], f_hats[i], n_w, True, p_ws[i], v_ws[i], u_ws[i], w_ws[i])
                st_idx += end_idx
                end_idx += n_div[i]

        # noinspection PyTypeChecker
        tournament(p_ws, v_ws, u_ws, w_ws, f_hats, n_w)
...
def analyze(n_w):
    ...
    try:
        ...
        while c < conf or not converged():
            ...
            # noinspection PyTypeChecker
            p_star, u_star, v_star, w_star, f_star_new = lcso(p, v, u, w, n_w)
            lagrs[time.monotonic()] = f_star_new

        with open('../perf-logs/lcso-convergence.log', 'w') as f:
            json.dump(lagrs, f)
        return True
    except Exception as e:
        print('[ERROR] LCSOConvergence analyze: Exception caught while analyzing the convergence properties of '
              'the Learning Competitive Swarm Optimization algorithm - {}'.format(traceback.print_tb(e.__traceback__)))
        return False


# Run Trigger
if __name__ == '__main__':
    print(f'[INFO] LCSOConvergence main: Learning Competitive Swarm Optimization | Data Length = {data_len} Mb | '
