...
# Simplified CPU/GPU version--with Progress Bar--and Multiple UAVs use-case with cell-sectoring...
...
# Utility Routine
def sys_progress(current_value, end_value, bar_length=100, log_level='DEBUG', entity='', routine='', key='Progress'):
    """
...
class UAVAngularVelocityOptimization(tfco.ConstrainedMinimizationProblem):
    ...
    def __init__(self, caller_id, wrapper, dual_variable, uav_radius_level, uav_radial_velocity_component,
        ...
        # print('[INFO] [{}] UAVAngularVelocityOptimization Initialization: Bringing things up...'.format(
        #     self.caller_id))
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        #       'Exception Value = {} | Traceback = {}'.format(self.caller_id, exc_type, exc_val,
...
class RandomTrajectoriesGeneration(object):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        #       f'Exception Type = {exc_type} | Exception Value = {exc_val} | Traceback = {traceback.print_tb(exc_tb)}')
...
class SMDPEvaluation(object):
    ...
    GROUND_NODE_DENSITY = 3.1831e-3  # The same density persists for a sectored-approach as well...
    ...
    ARRIVAL_RATE_PER_GROUND_NODE = 1.6667e-6
    ...
    PRIMAL_FEASIBILITY_THRESHOLD = 1e-6
    ...
    CSO_VELOCITY_DISCRETIZATION_LEVELS = 100
    ...
    # The Plotly API "lines-only" scatter plot mode
    PLOTLY_LINES_MODE = 'lines'
    ...
    UAV_POSITION_DISCRETIZATION_GRANULARITY = 100
    ...
    def __init__(self, id__, average_power_constraint, packet_length, canary_index, evaluators__):
        ...
        # Device placement: Waiting state & action space declarations
        ...
        # Device placement: Comm state & action space declarations
    ...
    def __distribute_ground_nodes(self):
        ...
        """
        plot_data, cell_radius, gn_density = list(), self.CELL_RADIUS, self.GROUND_NODE_DENSITY
        markers, lines, lines_markers = self.PLOTLY_MARKERS_MODE, self.PLOTLY_LINES_MODE, self.PLOTLY_LINES_MARKERS_MODE

        # number_of_gns = int(gn_density * np.pi * (cell_radius ** 2))
        number_of_gns = int(gn_density * ((np.pi * (cell_radius ** 2)) / 10))  # Multiple UAVs: 10 sectors

        radii = np.random.uniform(0, cell_radius, number_of_gns)

        # Multiple UAVs: The sector demarcation lines
        circ_angles = np.random.uniform(0, (2 * np.pi), (number_of_gns * 100))
        plot_data.append(graph_objs.Scatter(x=cell_radius * np.cos(circ_angles), y=cell_radius * np.sin(circ_angles),
                                            mode=markers, showlegend=False, marker=dict(size=1)))
        for angle in range(0, 360, 36):
            angle__ = angle * (np.pi / 180)
            plot_data.append(graph_objs.Scatter(x=radii * np.cos(angle__), y=radii * np.sin(angle__),
                                                mode=lines, showlegend=False))

        # angles = np.random.uniform(0, (2 * np.pi), number_of_gns)
        angles = np.random.uniform(0, (np.pi / 5), number_of_gns)  # Multiple UAVs: 10 sectors

        x_coords, y_coords = radii * np.cos(angles), radii * np.sin(angles)

        # Plotly plot of Ground Node distribution
        # plot_data = [graph_objs.Scatter(x=x_coords, y=y_coords, mode=markers)]

        # Multiple UAVs: 10 sectors
        plot_data.append(graph_objs.Scatter(x=x_coords, y=y_coords, mode=markers, showlegend=False))

        plot_layout = dict(title='Distribution of the GNs in a circular cell of radius {} m '
                                 '[Cartesian Coordinates]'.format(cell_radius),
        ...
        #       '{}'.format(self.id, cell_radius, plot_figure_url))

        print('[DEBUG] [{}] SMDPEvaluation distribute_ground_nodes: A plot representing the distribution of the '
              'Ground Nodes in a sector of area 10% that of a circular cell of radius a = {} m has been rendered, '
              'and is available here - {}'.format(self.id, cell_radius, plot_figure_url))  # Multiple UAVs: 10 sectors
    ...
    def __discretize_uav_positions(self, discretization_level, granularity_for_visualization):
        ...
        # v1 [Better Aesthetics]
        plot_data, cell_radius = list(), self.CELL_RADIUS
        markers, lines, lines_markers = self.PLOTLY_MARKERS_MODE, self.PLOTLY_LINES_MODE, self.PLOTLY_LINES_MARKERS_MODE

        # Class-wide initialization of a positional variable [A concurrency mandated op]
        self.uav_positions_polar = np.linspace(0, cell_radius, discretization_level, dtype=np.float64)

        # Multiple UAVs: The sector demarcation lines
        circ_angles = np.random.uniform((np.pi / 5), (2 * np.pi), (granularity_for_visualization * 100))
        plot_data.append(graph_objs.Scatter(x=cell_radius * np.cos(circ_angles), y=cell_radius * np.sin(circ_angles),
                                            mode=markers, showlegend=False, marker=dict(size=1)))
        for angle in range(0, 360, 36):
            angle__ = angle * (np.pi / 180)
            plot_data.append(graph_objs.Scatter(x=self.uav_positions_polar * np.cos(angle__),
                                                y=self.uav_positions_polar * np.sin(angle__), mode=lines,
                                                showlegend=False))

        for uav_radius_level in self.uav_positions_polar:
            # angles = np.random.uniform(0, (2 * np.pi), granularity_for_visualization)
            angles = np.random.uniform(0, (np.pi / 5), granularity_for_visualization)  # Multiple UAVs: 10 sectors

            x_coords, y_coords = uav_radius_level * np.cos(angles), uav_radius_level * np.sin(angles)

            # Class-wide initialization of a positional variable [A concurrency mandated op]
            self.uav_positions_rect = list(zip(x_coords, y_coords))

            # Plotly plot of UAV position discretization
            plot_data.append(graph_objs.Scatter(x=x_coords, y=y_coords, mode=markers, marker=dict(size=10),
                                                name='UAV Position Discretization Level: Radius = {}'.format(
                                                    np.round(uav_radius_level, 4))))

        plot_layout = dict(title='Discretized UAV Levels in a sector of radius {} m [Cartesian Coordinates]'.format(
            cell_radius), xaxis=dict(title='x [in m]'), yaxis=dict(title='y [in m]'))

        # v2 [Faster TF-based Processing - Not for plotting]
        ...
        # plot_figure = dict(data=[plot_data], layout=plot_layout)
        plot_figure = dict(data=plot_data, layout=plot_layout)  # Multiple UAVs: 10 sectors
        plot_figure_url = plotly.plotly.plot(plot_figure, filename='Odin_UAV_Position_Discretization',
                                             auto_open=False)
        ...
        print('[DEBUG] [{}] SMDPEvaluation discretize_uav_positions: A plot representing the discretization of UAV '
              'positions in a sector of area that is 10% that of a circular cell of radius a = {} m has been rendered, '
              'and is available here - {}'.format(self.id, cell_radius, plot_figure_url))  # Multiple UAVs: 10 sectors
    ...
    def __discretize_uav_radial_velocities(self, num_levels):
        ...
        print('[DEBUG] [{}] SMDPEvaluation __discretize_uav_radial_velocities: The discretized radial velocity '
              'levels, with V_max = {} m/s are {}'.format(self.id, self.MAX_UAV_VELOCITY,
                                                          [np.round(v, 4) for v in self.uav_radial_velocity_levels]))
    ...
    def __bisect(self, f, df, nc, y, low, high, tolerance):
        ...
        :raises AssertionError: Assertion failed.
        """
        # print('[TRACE] [{}] SMDPEvaluation __bisect: Inside bisection-based optimization for '
        #       'rate-adaptation...'.format(self.id))
        ...
        # print('[TRACE] [{}] SMDPEvaluation __bisect: Bisection-based optimization for '
        #       'rate-adaptation completed...'.format(self.id))
        return mid
    ...
    def __evaluate_los_throughput(self, p, d, phi, r_bar):
        ...
        """
        # print('[TRACE] [{}] SMDPEvaluation __evaluate_los_throughput: Calculating LoS throughput...'.format(self.id))
        ...
        z_star = self.__bisect(self.__f, df, nc, y, 0, self.__z((self.CHANNEL_BANDWIDTH *
        ...
        r_star = gamma_star * self.__marcum_q(df, nc, x_star)
    ...
    def __evaluate_nlos_throughput(self, p, d, r_bar):
        ...
        :param r_bar: The link throughput fed into this routine to capture the contribution of NLoS transmissions to
                      the total link throughput
        """
        # print('[TRACE] [{}] SMDPEvaluation __evaluate_nlos_throughput: Calculating NLoS throughput...'.format(
        #     self.id))
        ...
        # print('[TRACE] [{}] SMDPEvaluation __evaluate_nlos_throughput: NLoS throughput calculation '
    ...
    def __calculate_adapted_throughput(self, d, phi, r_bar):
        ...
        """
        # print('[TRACE] [{}] SMDPEvaluation __calculate_adapted_throughput: Calculating rate-adapted '
        #       'throughput...'.format(self.id))
        ...
        # print('[TRACE] [{}] SMDPEvaluation __calculate_adapted_throughput: Rate-adapted throughput calculation '
        #       'completed...'.format(self.id))
        # Nothing to return...
    ...
    def __interpolate_waypoints(self, p_indices, p, res_multiplier):
        ...
        """
        # print('[TRACE] [{}] SMDPEvaluation __interpolate_waypoints: Interpolating waypoints...'.format(self.id))

        m = len(p_indices)
        spl_x, spl_y = UnivariateSpline(p_indices, p[:, 0], s=0), UnivariateSpline(p_indices, p[:, 1], s=0)

        # print('[TRACE] [{}] SMDPEvaluation __interpolate_waypoints: Finished interpolating '
        #       'waypoints...'.format(self.id))
    ...
    def __interpolate_velocities(self, v_indices, v, res_multiplier):
        ...
        """
        # print('[TRACE] [{}] SMDPEvaluation __interpolate_velocities: Interpolating velocities...'.format(self.id))

        m, spl_v = len(v_indices), UnivariateSpline(v_indices, v, s=0)

        # print('[TRACE] [{}] SMDPEvaluation __interpolate_velocities: Finished interpolating '
        #       'velocities...'.format(self.id))
    ...
    def __penalties(self, p__, v__, x_g, res_multiplier):
        ...
        """
        # print('[TRACE] [{}] SMDPEvaluation __penalties: Inside penalties() method...'.format(self.id))
        ...
        with tf.device('/CPU:0'):
            ...
            with ThreadPoolExecutor(max_workers=d_gu.shape[0]) as executor:
        ...
        with tf.device('/CPU:1'):
            ...
            with ThreadPoolExecutor(max_workers=d_ub.shape[0]) as executor:
        ...
        # print('[TRACE] [{}] SMDPEvaluation __penalties: At the finish line of the penalties() '
        #       'method...'.format(self.id))
    ...
    def __calculate_comm_cost(self, p, v, nu, x_g, f_hat, e_hat=None, t_hat=None):
        ...
        """
        # print('[TRACE] [{}] SMDPEvaluation __calculate_comm_cost: Calculating comm state cost...'.format(self.id))
        ...
        # print('[TRACE] [{}] SMDPEvaluation __calculate_comm_cost: Comm state cost calculation '
        #       'completed...'.format(self.id))
        # Nothing to return...
    ...
    def __update_winners_and_losers(self, p, v, u, w, t_j, t_j_1, p_bar, v_bar, f_hats, nu, x_g):
        ...
        """
        # print('[TRACE] [{}] SMDPEvaluation __update_winners_and_losers: CSO winner-loser updates '
        ...
        # Device placement: Trajectory particles & associated particle velocities updates
        with tf.device('/CPU:0'):
        ...
        # Device placement: UAV velocity particles & associated particle velocities updates
        with tf.device('/CPU:1'):
    ...
    def __competitive_swarm_optimization(self, initial_uav_position, terminal_uav_position, ground_node_position,
        ...
        """
        # print('[DEBUG] [{}] SMDPEvaluation __competitive_swarm_optimization: CSO triggered...'.format(self.id))
        ...
        while k <= k_max:
            # print('[DEBUG] [{}] SMDPEvaluation __competitive_swarm_optimization: Inside CSO while '
            #       'loop...'.format(self.id))
            ...
            sys_progress(k, k_max, 20, self.__class__.__name__,
                         '__competitive_swarm_optimization', 'CSO progress per state-action pair')

            k += 1

        i = min(indices, key=lambda i__: f_hats[i__].numpy())

        # print('[DEBUG] [{}] SMDPEvaluation __competitive_swarm_optimization: CSO completed...'.format(self.id))
    ...
    def __angular_velocity_optimization(self, state_index, action_index, uav_position, radial_velocity,
        ...
        :param time_durations: The time-duration cost metrics collection for a value update associated with the fed-in
                               waiting state and action [concurrency requirement]
        """
        # print('[DEBUG] [{}] SMDPEvaluation __angular_velocity_optimization: Angular Velocity Optimization '
        #       'triggered...'.format(self.id))
        ...
        theta_c_1, theta_c = tf.Variable(np.inf), tf.Variable(0.0)  # API forced
        ...
        with UAVAngularVelocityOptimization(id__, self, nu, r_u, v_r, theta_c) as problem:
            ...
            while (abs(theta_c - theta_c_1) >= delta_c) or (conf <= conf_th):
                tf.compat.v1.assign(theta_c_1, theta_c, validate_shape=True, use_locking=False)
    ...
    def __hierarchical_competitive_swarm_optimization(self, state_index, action_index, initial_uav_position,
        ...
        """
        # print('[DEBUG] [{}] SMDPEvaluation __hierarchical_competitive_swarm_optimization: '
        #       'HCSO triggered...'.format(self.id))
        ...
        #
        # # Device Placement: Random tensor generation for UAV velocity particles and particle velocities collections
        # #   for the trajectories tensor and the UAV velocities tensor, for HCSO initialization
        # with tf.device('/CPU:1'):
        #     v, u, w = [tf.Variable(np.random.choice(np.linspace(v_min, v_max, v_levels, dtype=np.float64),
        #                                             size=[n, m]))] * 3

        p = tf.Variable([random.choices(self.uav_positions_rect, k=m) for _ in range(n)])
        v, u, w = [tf.Variable(np.random.choice(np.linspace(self.CSO_MINIMUM_VELOCITY_VALUE, self.MAX_UAV_VELOCITY,
                                                            self.CSO_VELOCITY_DISCRETIZATION_LEVELS), size=[n, m]))] * 3

        # HCSO while loop
        while m <= m_max:
            # print('[DEBUG] [{}] SMDPEvaluation __hierarchical_competitive_swarm_optimization: Inside HCSO while '
            #       'loop...'.format(self.id))
            ...
            # Device Placement: Trajectory particles
            with tf.device('/CPU:0'):
            ...
            # Device Placement: UAV velocity particles
            with tf.device('/CPU:1'):
                shape_v = [n, (m_ip * m)]
            ...
            sys_progress(m, (m_max * 2), 20, 'INFO', self.__class__.__name__,
                         '__hierarchical_competitive_swarm_optimization', 'HCSO progress per state-action pair')
        ...
        # Device Placement: Tensor updates for further processing
        with tf.device('/CPU:0'):
        ...
        # Device Placement: Class-wide data collection updates for visualization
        with tf.device('/CPU:1'):
    ...
    def __optimize_waiting_states(self, nu, s_wait, a_wait, l_wait_star, e_wait_star, t_wait_star):
        ...
        :param t_wait_star: The temporal costs (final optimality) collection (tensor) for waiting states and actions
                            ($t_{\nu}^{*}(s; v_{r}, \theta_{c}^{*})$)
        """
        # print('[DEBUG] [{}] SMDPEvaluation __optimize_waiting_states: Waiting states optimization '
        #       'started...'.format(self.id))

        with ThreadPoolExecutor(max_workers=(s_wait.shape[0] * a_wait.shape[0])) as executor:
    ...
    def __optimize_comm_states(self, nu, s_comm, a_comm, l_comm_star, e_comm_star, t_comm_star):
        ...
        :param t_comm_star: The temporal costs (final optimality) collection (tensor) for comm states and actions
                            ($t_{\nu}^{*}(s; \hat{\mathbf{x}}_{U}, 0 or 1)$)
        """
        # print('[DEBUG] [{}] SMDPEvaluation __optimize_comm_states: Comm states optimization started...'.format(
        #     self.id))

        with ThreadPoolExecutor(max_workers=(s_comm.shape[0] * a_comm.shape[0])) as executor:
        ...
        # print('[DEBUG] [{}] SMDPEvaluation __optimize_comm_states: Comm states optimization '
        #       'completed...'.format(self.id))
        # Nothing to return...
    ...
    def __smdp_waiting_viter_updates(self, l_wait_star, e_wait_star, t_wait_star, v_i_wait, v_i_comm, h_i_wait,
        ...
        """
        # print('[DEBUG] [{}] SMDPEvaluation __smdp_waiting_viter_updates: SMDP VITER updates started...'.format(
        #     self.id))
        ...
        # Device placement: Waiting state value functions: Transition to another waiting state
        with tf.device('/CPU:0'):
            v_i_wait_remapped = tf.gather(tf.tile(tf.expand_dims(v_i_wait, axis=1), multiples=[1, a_wait.shape[0]]),
                                          wait_indices)[:, :, 0]

        # Device placement: Comm state value functions: Transition to a comm state
        with tf.device('/CPU:1'):
        ...
        # Device placement: Waiting state energy & time costs
        with tf.device('/CPU:0'):
        ...
        # Device placement: Comm state energy & time costs
        with tf.device('/CPU:1'):
        ...
        # Device placement: Delay (time duration) cost
        with tf.device('/CPU:1'):
        ...
        # print('[DEBUG] [{}] SMDPEvaluation __smdp_waiting_viter_updates: SMDP VITER updates '
        #       'completed...'.format(self.id))
        # Nothing to return
    ...
    def __smdp_comm_viter_updates(self, l_comm_star, e_comm_star, t_comm_star, v_i_comm, v_i_wait, h_i_comm,
        ...
        """
        # print('[DEBUG] [{}] SMDPEvaluation __smdp_comm_viter_updates: SMDP VITER updates started...'.format(
        #     self.id))
        ...
        # Device Placement: Comm state energy costs
        with tf.device('/CPU:0'):
        ...
        # Device Placement: Comm state time duration costs
        with tf.device('/CPU:1'):
        ...
        # print('[DEBUG] [{}] SMDPEvaluation __smdp_comm_viter_updates: SMDP VITER updates completed...'.format(
        #     self.id))
        # Nothing to return
    ...
    def __value_iteration(self, dual_variable):
        ...
        """
        # print('[DEBUG] [{}] SMDPEvaluation __value_iteration: SMDP Value Iteration started...'.format(self.id))

        i, nu, delta, canary_index = 0, dual_variable, self.VITER_TERMINATION_THRESHOLD, self.canary_state_index
        conf, conf_th = 0, self.CONVERGENCE_CONFIDENCE

        # Device Placement: Waiting states
        with tf.device('/CPU:0'):
        ...
        max_workers_o = ((s_wait_size * a_wait_size) + (s_comm_size * a_comm_size))
        max_workers_a = (s_wait_size + s_comm_size)
        ...
        # print('[DEBUG] [{}] SMDPEvaluation __value_iteration: SMDP Value Iteration completed...'.format(self.id))
    ...
    def projected_subgradient_ascent(self):
        ...
        """
        print('[DEBUG] [{}] SMDPEvaluation projected_subgradient_ascent: Projected Sub-Gradient Ascent '
              'started...'.format(self.id))
        ...
        while (abs(g_k - g_k_1) >= th_d) or (e_k - (p_av * t_k) >= th_pf) or (nu_k * abs(e_k - (p_av * t_k)) >= th_cs):
            print('[DEBUG] [{}] SMDPEvaluation projected_subgradient_ascent: A new iteration of the '
    ...
    def prepare_waiting_state_policy_data(self, waiting_policies):
        ...
        :param waiting_policies: The collection of plot traces for drawing the optimal waiting state policy plot
                                 [Concurrency Op]
        """
        print('[DEBUG] [{}] SMDPEvaluation prepare_waiting_state_policy_data: Waiting state policy data...')
    ...
    def prepare_delay_power_tradeoff_data(self, trade_offs):
        ...
        :param trade_offs: The collection of plot traces for illustrating the delay-power trade-off in comm states
                           [Concurrency Op]
        """
        print('[DEBUG] [{}] SMDPEvaluation prepare_delay_power_tradeoff_data: '
              'Delay-Power trade-off data...'.format(self.id))

        p_len, pwr, t = self.packet_length, self.average_power_constraint, self.canary_comm_average_delay

        with lock:
            trace = trade_offs.get(p_len, graph_objs.Scatter(x=list(), y=list(), mode=self.PLOTLY_LINES_MARKERS_MODE,
    ...
    def prepare_no_relay_gns_data(self):
        ...
        """
        print('[DEBUG] [{}] SMDPEvaluation prepare_no_relay_gns_data: No relay GNs data...'.format(self.id))
    ...
    def prepare_uav_trajectories_data(self):
        ...
        """
        print('[DEBUG] [{}] SMDPEvaluation prepare_uav_trajectories_data: UAV trajectories data...'.format(self.id))
    ...
    def prepare_lagrangian_convergence_data(self):
        ...
        :return A plotly scatter object constituting the convergence of the Lagrangian cost metrics in the comm states
                ($l_{\nu}^{*}(s;\hat{r}_{U}, 1)$ v Time [s])
        """
        print('[DEBUG] [{}] SMDPEvaluation prepare_lagrangian_convergence_data: '
              'Lagrangian convergence data...'.format(self.id))
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        print('[INFO] [{}] SMDPEvaluation Termination: Tearing things down - Exception Type = {} | Exception Value = {}'
              ' | Traceback = {}'.format(self.id, exc_type, exc_val, traceback.print_tb(exc_tb)))
...
def plot_waiting_state_policy(pwr, plot_traces):
    ...
    """
    print('[DEBUG] [Main Thread] SMDPEvaluation plot_waiting_state_policy: Plot waiting state policy...')
...
def plot_delay_power_tradeoff(plot_traces_dict):
    ...
    """
    print('[DEBUG] [Main Thread] SMDPEvaluation plot_delay_power_tradeoff: Plot delay-power trade-off...')
...
def plot_no_relay_gns(pwr, plot_traces):
    ...
    """
    print('[DEBUG] [Main Thread] SMDPEvaluation plot_no_relay_gns: Plot no-relay GNs...')
...
def plot_uav_trajectories(canary, plen, pwr, plot_trace):
    ...
    """
    print('[DEBUG] [Main Thread] SMDPEvaluation plot_uav_trajectories: Plot UAV trajectories...')
...
def plot_lagrangian_convergence(canary, plen, pwr, plot_trace):
    ...
    """
    print('[DEBUG] [Main Thread] SMDPEvaluation plot_lagrangian_convergence: Plot lagrangian convergence...')
...
if __name__ == '__main__':
    ...
    canary_idx = 100  # The index of the canary state (in the comm state space) for visualizations
    ...
    max_workers = (avg_powers.shape[0] * packet_lens.shape[0])
    ...
    print('[DEBUG] [Main Thread] SMDPEvaluation main: SMDPEvaluation completed: Starting Phase-I Visualizations...')
    ...
    print('[DEBUG] [Main Thread] SMDPEvaluation main: Phase-I Visualizations completed: Starting Phase-II '
          'Visualizations...')
