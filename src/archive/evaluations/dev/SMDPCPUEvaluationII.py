...
The plots involved in this evaluation are:
    1. UAV Radial Velocity ($v_{r}$ m/s) v UAV Radius Level ($r_{U}$ m) w.r.t the optimal waiting state policy, for
       L = 0.1/1.0, 5.0, and 10.0 Mbits;
    2. Expected Average Delay ($\Bar{T}$ s) v UAV Average Power Constraint ($P_{\text{avg}}$ W) w.r.t the optimal
       policy, for L = 0.1/1.0, 5.0, and 10.0 Mbits;
    3. Map of GNs ($x$ m v $y$ m) that transmit directly to the BS w.r.t the optimal policy, for L = 0.1/1.0, 5.0, and
       10.0 Mbits;
...
Version v21.09: Mathematical Modeling by Matthew Bliss <mbliss@purdue.edu> and Nicolo Michelusi <michelus@purdue.edu>
...
"""
Release: Final CPU version, with Progress Bar: No device placement | Matt's parameters | JSAC SI 2021 Ordering
"""
...
os.environ['TF_XLA_FLAGS'] = '--tf_xla_auto_jit=2 --tf_xla_cpu_global_jit ' \
                             '/home/bkeshav1/workspace/repos/Odin/src/uav-mobility/core/SMDPCPUEvaluationII.py'
...
class RandomTrajectoriesGeneration(object):
    ...
    def __optimize_segment(self, traj, opt_traj):
        ...
        tf.compat.v1.assign(opt_traj, tf.clip_by_norm(tf.constant(list(zip(
            ...
            UnivariateSpline(i_s, tf.concat([x_0[:, 1], traj[:, 1], x_m[:, 1]], axis=0), s=0)(x)))),
            a, axes=1), validate_shape=True, use_locking=True)
...
class SMDPEvaluation(object):
    ...
    DUAL_CONVERGENCE_THRESHOLD = 1e-5
    ...
    COMPLEMENTARY_SLACKNESS_THRESHOLD = 0.5
    ...
    def __init__(self, id__, average_power_constraint, packet_length, canary_index, evaluators__):
        ...
        self.o_star, self.u_star, self.u_star_indices, self.canary_comm_average_delay = None, None, None, 0.0
    ...
    def __calculate_comm_cost(self, p, v, nu, x_g, f_hat, e_hat=None, t_hat=None):
        ...
        t = tf.divide(tf.norm(tf.roll(p, shift=-1, axis=0)[:-1, :] - p[:-1, :], axis=1),
                      tf.where(tf.equal(v[:-1], 0.0), tf.ones_like(v[:-1]), v[:-1]))
        e__, t__ = tf.reduce_sum(tf.multiply(t, self.__power_cost(v[:-1]))), tf.reduce_sum(t)
    ...
    def __hierarchical_competitive_swarm_optimization(self, state_index, action_index, initial_uav_position,
        ...
        with RandomTrajectoriesGeneration(x_0, x_m, (-a, a), (0, 2 * np.pi), n, m_old, m_ip) as generator:
    ...
    def __optimize_waiting_states(self, nu, s_wait, a_wait, l_wait_star, e_wait_star, t_wait_star):
        ...
        try:
            ...
            print('[ERROR] [{}] SMDPEvaluation __optimize_waiting_states: Exception caught during waiting '
    ...
    def __optimize_comm_states(self, nu, s_comm, a_comm, l_comm_star, e_comm_star, t_comm_star):
        ...
        :param t_comm_star: The temporal costs (final optimality) collection (tensor) for comm states and actions
                            ($t_{\nu}^{*}(s; \hat{\mathbf{x}}_{U}, 0 or 1)$)
        """
        print('[DEBUG] [{}] SMDPEvaluation __optimize_comm_states: '
              'Comm states optimization started...'.format(self.id))
        try:
            with ThreadPoolExecutor(max_workers=256) as executor:
            ...
            print('[ERROR] [{}] SMDPEvaluation __optimize_comm_states: Exception caught during comm '
                  'state optimization - {}'.format(self.id, traceback.print_tb(e__.__traceback__)))
    ...
    def __smdp_waiting_viter_updates(self, l_wait_star, e_wait_star, t_wait_star, v_i_wait, v_i_comm, h_i_wait,
        ...
        """
        print('[DEBUG] [{}] SMDPEvaluation __smdp_waiting_viter_updates: '
              'SMDP VITER updates started...'.format(self.id))
        try:
            ...
            print(
                '[ERROR] [{}] SMDPEvaluation __smdp_waiting_viter_updates: Exception caught during SMDP VITER waiting '
    ...
    def __smdp_comm_viter_updates(self, l_comm_star, e_comm_star, t_comm_star, v_i_comm, v_i_wait, h_i_comm,
        ...
        """
        print('[DEBUG] [{}] SMDPEvaluation __smdp_comm_viter_updates: '
              'SMDP VITER updates started...'.format(self.id))
        try:
            ...
            print('[ERROR] [{}] SMDPEvaluation __smdp_comm_viter_updates: Exception caught during SMDP VITER comm '
                  'state updates - {}'.format(self.id, traceback.print_tb(e__.__traceback__)))
    ...
    def __value_iteration(self, dual_variable):
        ...
        try:
            ...
            print('[ERROR] [{}] SMDPEvaluation __value_iteration: Exception caught during value iteration '
    ...
    def projected_subgradient_ascent(self):
        ...
        try:
            ...
            while not converged or conf < conf_th:
                ...
                conf += 1 if converged else -conf  # Update OR Reset if it is a red herring
                k += 1
            self.o_star, self.canary_comm_average_delay = o_star_k, t_k
            self.u_star, self.u_star_indices = u_star_k, u_star_indices_k
            print('[DEBUG] [{}] SMDPEvaluation projected_subgradient_ascent: Projected Sub-Gradient Ascent '
                  'completed...'.format(self.id))
        except Exception as e__:
            print('[ERROR] [{}] SMDPEvaluation projected_subgradient_ascent: Exception caught during projected '
    ...
    def prepare_waiting_state_policy_data(self, waiting_policies):
        ...
        :param waiting_policies: The collection of plot traces for drawing the optimal waiting state policy plot
                                 [Concurrency Op]
        """
        print('[DEBUG] [{}] SMDPEvaluation prepare_waiting_state_policy_data: '
              'Waiting state policy data...'.format(self.id))
        p_len, s_wait, o_star = self.packet_length / 1e6, self.waiting_states, self.o_star
        with lock:
            waiting_policies.append(graph_objs.Scatter(x=s_wait.numpy(), y=o_star.numpy(),
                                                       mode=self.PLOTLY_LINES_MARKERS_MODE,
    ...
    def prepare_delay_power_tradeoff_data(self, trade_offs):
        ...
        with lock:
            x, y = trade_offs.get(p_len, [[], []])
            x.append(pwr)
            y.append(t)
            trade_offs[p_len] = [x, y]

    def prepare_no_relay_gns_data(self, idx_, gn_maps__):
        """
        Map of GNs ($x$ m v $y$ m) that transmit directly to the BS w.r.t the optimal policy, for L = 0.1/1.0, 5.0, and
        10.0 Mbits

        :param idx_: An index parameter for plot trace differentiation
        :param gn_maps__: A plotly plot trace list to be populated with a map of GNs that do not use the UAV as a relay
                          [A concurrency mandated op]
        ...
        p_len, canary_state = self.packet_length, self.comm_states[self.canary_state_index]
        indices = tf.reshape(tf.where(tf.reduce_all(tf.equal(self.comm_states[:, 0], canary_state[0]), axis=1)),
                             shape=[-1])
        gns_0_x, gns_0_y = list(), list()
        for i_ in indices:
            if self.relay_status[i_] == 0:
                x, y = self.comm_states[i_, 1].numpy()
                gns_0_x.append(x)
                gns_0_y.append(y)
        if idx_ == 0:
            # Plot UAV position
            gn_maps__.append(graph_objs.Scatter(x=[canary_state[0, 0].numpy()], y=[canary_state[0, 1].numpy()],
                                                mode=self.PLOTLY_MARKERS_MODE,
                                                marker=dict(symbol=raw_symbols[0], color=raw_colors[0], size=30),
                                                name=r'UAV Position $(x_{U}, y_{U}){=}$' +
                                                     str(canary_state[0].numpy())))
        # Plot GNs using Direct BS communication
        gn_maps__.append(graph_objs.Scatter(x=gns_0_x, y=gns_0_y, mode=self.PLOTLY_MARKERS_MODE,
                                            marker=dict(symbol=raw_symbols[idx_ + 1],
                                                        color=raw_colors[idx_ + 1], size=20),
    ...
    def prepare_uav_trajectories_data(self):
        ...
        i, idx = 0, self.canary_state_index
        u_star_indices, canary_state, plot_data = self.u_star_indices, self.comm_states[idx], list()
        indices = tf.reshape(tf.where(tf.reduce_all(tf.equal(self.comm_states[:, 0], canary_state[0]), axis=1)),
                             shape=[-1])
        states = tf.gather(self.comm_states, indices, axis=0)
        # Plot UAV position
        plot_data.append(graph_objs.Scatter(x=[canary_state[0, 0].numpy()], y=[canary_state[0, 1].numpy()],
                                            mode=self.PLOTLY_MARKERS_MODE,
                                            marker=dict(symbol=raw_symbols[0], color=raw_colors[0], size=30),
                                            name=r'UAV Initial Position $(x_{0}, y_{0}){=}$' +
                                                 str(canary_state[0].numpy())))
        for state in states:
            a_optimal = u_star_indices[indices[i]]
            opt_traj = self.optimal_trajectories[indices[i], a_optimal]
            i += 1  # GN identifier
            symbol, color = raw_symbols[i], raw_colors[i]
            # Plot GN_{i} position
            plot_data.append(graph_objs.Scatter(x=[state[1, 0].numpy()], y=[state[1, 1].numpy()],
                                                mode=self.PLOTLY_MARKERS_MODE,
                                                marker=dict(symbol=symbol, color=color, size=20),
                                                name=f'Ground Node {i}: ' + r'$(x_{G}, y_{G}){=}$' +
                                                     str(state[1].numpy())))
            # Plot the associated optimal trajectory
            plot_data.append(graph_objs.Scatter(x=opt_traj[:, 0].numpy(), y=opt_traj[:, 1].numpy(),
                                                mode=self.PLOTLY_MARKERS_MODE,
                                                marker=dict(symbol=symbol, color=color, size=5),
    ...
    def prepare_lagrangian_convergence_data(self):
        ...
        t, lagr = list(self.canary_state_comm_costs.keys()), list(self.canary_state_comm_costs.values())
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_tb is not None:
            print(
                '[ERROR] [{}] SMDPEvaluation Termination: Tearing things down - Exception Type = {} | '
...
def plot_delay_power_tradeoff(plot_traces_dict):
    ...
    :param plot_traces_dict: The plot data traces dictionary {L: ($\Bar{T}$ s v $P_{\text{avg}}$ W) for different
                             values of the data packet length ($L$), passed to the Plotly API
    """
...
if __name__ == '__main__':
    ...
    e_idx, canary_idx = 0, 2021  # The index of the canary state (in the comm state space) for visualizations
    # Test samples
    avg_powers, packet_lens = np.array([2e3]), np.array([10e6])
    ...
    try:
        with ThreadPoolExecutor(max_workers=256) as exxeggutor:
            exxeggutor.submit(plot_waiting_state_policy, 1.4e3, wait_policies)  # Fig. 2 (b)
            exxeggutor.submit(plot_delay_power_tradeoff, t_p_tradeoffs)  # Fig. 4 (a), (b), and (c)
            exxeggutor.submit(plot_no_relay_gns, 1.4e3, gn_maps)  # Fig. 3 (a)
    except Exception as e:
