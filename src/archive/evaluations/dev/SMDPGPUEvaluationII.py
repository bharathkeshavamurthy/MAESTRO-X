...
plotly.tools.set_credentials_file(username='bkeshava', api_key='JXwZnV9nT4r85rES5esy')  # User Credentials
...
class RandomTrajectoriesGeneration(object):
    ...
    def __init__(self, caller_id, source, destination, radial_bounds, angular_bounds, swarm_size,
        ...
        # print('[INFO] [{}] RandomTrajectoriesGeneration Initialization: Bringing things up...'.format(self.caller_id))
    ...
    def generate(self):
        ...
        with ThreadPoolExecutor(max_workers=(25 * n)) as executor:
            for i in range(n):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        """
        if exc_tb is not None:
            print('[INFO] [{}] RandomTrajectoriesGeneration Termination: Tearing things down - Exception Type = {} | '
                  'Exception Value = {} | Traceback = {}'.format(self.caller_id, exc_type, exc_val,
                                                                 traceback.print_tb(exc_tb)))
...
class SMDPEvaluation(object):
    ...
    ARRIVAL_RATE_PER_GROUND_NODE = 5.5556e-3
    ...
    UAV_POSITION_DISCRETIZATION_GRANULARITY = 1
    ...
    def __discretize_uav_radial_velocities(self, num_levels):
        ...
        print('[DEBUG] [{}] SMDPEvaluation __discretize_uav_radial_velocities: The discretized radial velocity '
              'levels, with V_max = {} m/s are {}'.format(self.id, self.MAX_UAV_VELOCITY,
                                                          [v for v in self.uav_radial_velocity_levels]))
    ...
    def __interpolate_waypoints(self, p_indices, p, res_multiplier):
        ...
        # print('[TRACE] [{}] SMDPEvaluation __interpolate_waypoints: Finished interpolating waypoints...'.format(
        #     self.id))
    ...
    def __penalties(self, p__, v__, x_g, res_multiplier):
        ...
        with tf.device('/GPU:0'):
            ...
            with ThreadPoolExecutor(max_workers=(25 * d_gu.shape[0])) as executor:
        ...
        with tf.device('/GPU:1'):
            ...
            with ThreadPoolExecutor(max_workers=(25 * d_ub.shape[0])) as executor:
    ...
    def __competitive_swarm_optimization(self, state_index, action_index, initial_uav_position, terminal_uav_position,
                                         ground_node_position, trajectory_particles, uav_velocity_particles,
        ...
        :param state_index: The index of the comm state being optimized [debugging requirement]
        :param action_index: The index of the comm state action being optimized [debugging requirement]
        ...
        while k <= k_max:
            ...
            print('[DEBUG] [{}] SMDPEvaluation __competitive_swarm_optimization: '
                  '[{} | {}] - {}%'.format(self.id, i__, j__, ((k / k_max) * 100)))
    ...
    def __hierarchical_competitive_swarm_optimization(self, state_index, action_index, initial_uav_position,
        ...
        zeta, m, m_max = self.HCSO_TRAJECTORY_SCALING_FACTOR, (m_ip * (m_old + 2)), self.MAXIMUM_TRAJECTORY_SEGMENTS
        ...
        while m <= self.MAXIMUM_TRAJECTORY_SEGMENTS:
            print(
                '[DEBUG] [{}] SMDPEvaluation __hierarchical_competitive_swarm_optimization: Inside HCSO while '
                'loop...'.format(self.id))

            p_star, v_star, f_star = self.__competitive_swarm_optimization(i__, j__, x_0, x_m, x_g, p, v,
                                                                           u, w, n, m, nu)  # CSO call
            ...
            print('[DEBUG] [{}] SMDPEvaluation __hierarchical_competitive_swarm_optimization: '
                  '[{} | {}] - {}%'.format(self.id, i__, j__, ((m / (m_max * 2)) * 100)))

        p_star_2, v_star_2, f_star_2 = self.__competitive_swarm_optimization(i__, j__, x_0, x_m, x_g, p, v,
                                                                             u, w, n, m, nu)  # CSO call
        # Save the Lagrangian cost metric's value at this stage for Cost-Computation Convergence
        if eval__:
    ...
    def __optimize_waiting_states(self, nu, s_wait, a_wait, l_wait_star, e_wait_star, t_wait_star):
        ...
        with ThreadPoolExecutor(max_workers=(25 * s_wait.shape[0] * a_wait.shape[0])) as executor:
    ...
    def __optimize_comm_states(self, nu, s_comm, a_comm, l_comm_star, e_comm_star, t_comm_star):
        ...
        :param t_comm_star: The temporal costs (final optimality) collection (tensor) for comm states and actions
                            ($t_{\nu}^{*}(s; \hat{\mathbf{x}}_{U}, 0 or 1)$)
        """
        print('[DEBUG] [{}] SMDPEvaluation __optimize_comm_states: Comm states optimization started...'.format(self.id))

        with ThreadPoolExecutor(max_workers=(25 * s_comm.shape[0] * a_comm.shape[0])) as executor:
    ...
    def __smdp_waiting_viter_updates(self, l_wait_star, e_wait_star, t_wait_star, v_i_wait, v_i_comm, h_i_wait,
        ...
        """
        print('[DEBUG] [{}] SMDPEvaluation __smdp_waiting_viter_updates: SMDP VITER updates started...'.format(self.id))
    ...
    def __smdp_comm_viter_updates(self, l_comm_star, e_comm_star, t_comm_star, v_i_comm, v_i_wait, h_i_comm,
        ...
        """
        print('[DEBUG] [{}] SMDPEvaluation __smdp_comm_viter_updates: SMDP VITER updates started...'.format(self.id))
        ...
        print('[DEBUG] [{}] SMDPEvaluation __smdp_comm_viter_updates: SMDP VITER updates completed...'.format(self.id))
    ...
    def __value_iteration(self, dual_variable):
        ...
        max_workers_o = 50 * ((s_wait_size * a_wait_size) + (s_comm_size * a_comm_size))
        max_workers_a = 50 * (s_wait_size + s_comm_size)
...
def plot_delay_power_tradeoff(plot_traces_dict):
    ...
    for plen, plot_trace in plot_traces_dict.items():
        ...
        fig = dict(data=[graph_objs.Scatter(x=xs, y=ys, mode='lines+markers',
                                            name='Delay-Power TradeOff: ' + r'L{=}$' + str(plen) + ' Mbits')],
                   layout=plot_layout)
...
def plot_uav_trajectories(canary, plen, pwr, plot_trace):
    ...
    print('[INFO] [Main Thread] SMDPEvaluation plot_uav_trajectories: The plot of the optimal HCSO-determined UAV '
          ...
          'Initial UAV Position = {} [m, m], is available here - {}'.format(plen, pwr, canary[0].numpy(), fig_url))
...
def plot_lagrangian_convergence(canary, plen, pwr, plot_trace):
    ...
    plot_layout = dict(title='Lagrangian Cost Convergence for a canary comm state, with data packet length '
                       ...
                       yaxis=dict(title=r'$(1{-}\nu P_{\text{avg}})\Delta + \nu E$ (in J)', type='log', autorange=True))
    fig = dict(data=[plot_trace], layout=plot_layout)
...
if __name__ == '__main__':
    ...
    e_idx, canary_idx = 0, 18  # The index of the canary state (in the comm state space) for visualizations
    ...
    packet_lens = np.array([0.1e6, 1e6, 5e6, 10e6])

    # Test samples
    # avg_powers, packet_lens = np.array([1.2e3]), np.array([1e6])
    ...
    try:
        with ThreadPoolExecutor(max_workers=max_workers) as exxeggutor:
            for evaluator in evaluators:
                ...
                exxeggutor.submit(evaluator.prepare_delay_power_tradeoff_data, t_p_tradeoffs)  # Delay-Power data

                if avg_p == 1.2e3:
                    exxeggutor.submit(evaluator.prepare_waiting_state_policy_data, wait_policies)  # Wait Policy Data

                    exxeggutor.submit(evaluator.prepare_no_relay_gns_data, e_idx, gn_maps)  # Direct BS GN data
                    e_idx += 1
        ...
        print('[ERROR] [Main Thread] SMDPEvaluation main: Exception caught during Phase-I evaluations: {}'.format(
    ...
    try:
        ...
        print('[ERROR] [Main Thread] SMDPEvaluation main: Exception caught during Phase-II evaluations: {}'.format(
            traceback.print_tb(exc.__traceback__)))
