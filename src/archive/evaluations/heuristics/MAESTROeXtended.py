...
(final-augmented): JFQ at the BS | PHY & DLL prescient scheduler between the BS and the UAV fleet...

For the "preliminary" policy evaluation, see MAESTROAlpha.py.
For the "preliminary-augmented" ("wait?return-to-center") policy evaluation, see MAESTROBeta.py.
...
def wait(x_u, z_u):
    ...
    d_u = tf.multiply(tf.abs(o_star[tf.argmin(s_wait - r_u)]), t_u)
    # Based on the sign of the optimal radial velocity action, determine which radii-levels (r^) to consider
    if o_star[r_u] < 0.0:
...
def gn_request(z, x_g, deltas):
    ...
    x_g: The (x, y) location of the GN originating the current request to be served in our system
    deltas: A collection of service times for each BS channel & UAV index, for evaluating the JFQ logic
    """
    t = env.now
    ...
    # BS JFQ-heuristic: Join the fastest channel-queue | UAV Conflict Resolution
    bs_tmps = [(ch, bs_delta_s[st[0]], bs_energy_s[st[0]],
                omi * (bs_delta_s[st[0]] + deltas[ch]) + (1 - omi) * bs_energy_s[st[0]], None)
               for ch in range(number_of_bs_channels)]  # BS temporary variables: Index, D, E, D/E Value, No Endpoint
    uav_tmps = [(number_of_bs_channels + u, uav_delta_s[s], uav_energy_s[s],
                 omi * (uav_delta_s[s] + deltas[u + number_of_bs_channels]) + (1 - omi) * uav_energy_s[s],
                 u_star[s]) for u, s in enumerate(st)]  # UAV(s) temporary variables: Index, D, E, D/E Value, Endpoint
    k, mu, nrg, de_val, endp = min(np.concatenate(bs_tmps, uav_tmps), key=lambda tmp: tmp[3])  # D/E: Conflict Res + JFQ
    deltas[k] += mu  # After resolving the best BS channel / UAV index, append the service time to the deltas dictionary
    ...
    if k >= number_of_bs_channels:
        z[k] = time.monotonic()
        tf.compat.v1.assign(uav_coords[k - number_of_bs_channels], endp, validate_shape=True, use_locking=True)
...
def arrivals():
    ...
    for i in range(n_comm):
        env.process(gn_request(z, s_comm[i_s[i], 1], deltas))
...
try:
    ...
    print(f'[ERROR] MAESTROeXtended main: Exception caught while parsing {filename} - '
...
print(f'[INFO] MAESTROeXtended main: MAESTRO eval ID = {uid.numpy().decode()} | Number of UAVs = {number_of_uavs} |'
      ...
      f' 1 UAV-relay average service time = {np.mean(delta_s)} s |'
      f' 1 UAV-relay average energy consumption = {np.mean(energy_s) / 1e3} kJ |'
