...
pi = np.pi
np.random.seed(6)
num_levels, min_dist = 6, 240.0
a, m, m_ip, n, num_uavs = 1e3, 14, 2, int(1e4), 1
a_los, a_nlos, kappa, m_post = 2.0, 2.8, 0.2, m_ip * (m + 2)
pc, ra_conf, ra_tol, cx_conf, cx_tol = 90.0, 3, 1e-3, 3, 1e-3
depl_env, rf, le_l, le_m, le_h = 'rural', num_uavs, 1, 10, 100
r_bounds, th_bounds, h_uavs, h_gns = (-a, a), (0, 2 * pi), 200.0, 0.0
max_iters, eps_abs, eps_rel, warm_start, verbose = int(1e3), 1e-3, 1e-3, True, True
data_len, arr_rates_r = [1e6, 10e6, 100e6][1], {1e6: 5 / 60, 10e6: 1 / 60, 100e6: 1 / 360}
...
def evaluate():
    ...
    w_times, s_times, env = [], [], Environment()

    env.process(arrivals(env, [Resource(env) for _ in range(n_c)],
                         n, arr_rates[data_len], w_times, s_times, serv_times))
    ...
    print('[DEBUG] SCAEvaluation evaluate: '
          f'{n_u} UAV-relays | M/G/{n_c} queuing at the data channels | '
