...
class DeterministicTrajectoriesGeneration(object):
    ...
    def __init__(self, a, m_post, x_0, x_g, x_m):
        self.a = a
        self.x_0 = x_0
        self.x_g = x_g
        self.x_m = x_m
...
class MAESTRO(object):
    ...
    def __optimize_trajectories(self, state_index, action_index,
                                c_state, c_action, dual_variable, lagrangian, energy, time_duration):
        nu, f_hat = dual_variable, tf.Variable(0.0, dtype=tf.float64)
        m_max, m_ipf = self.MAX_TRAJECTORY_SEGMENTS, self.INTERPOLATION_FACTOR
        ...
        a, m_pos = self.CELL_RADIUS, m_max * m_ipf
        ...
        p_star = tf.Variable(tf.zeros(shape=[m_pos, 2], dtype=tf.float64), dtype=tf.float64)
            ...
            with ThreadPoolExecutor(max_workers=n_w) as executor:
                for i__, s in enumerate(s_comm):
                    for j__, r_u in enumerate(a_comm):
                        executor.submit(self.__optimize_trajectories,
