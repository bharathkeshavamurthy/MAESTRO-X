...
# CPU version for Intel Xeon Gold Run on Emulab
...
class RandomTrajectoriesGeneration(object):
    ...
    def generate(self):
        ...
        with ThreadPoolExecutor(max_workers=(100 * n)) as executor:
            for i in range(n):
...
class SMDPEvaluation(object):
    ...
    VITER_TERMINATION_THRESHOLD = 1e-2
    ...
    #   waiting states
    ANGULAR_VELOCITY_TERMINATION_THRESHOLD = 1e-2
    ...
    CSO_VELOCITY_DISCRETIZATION_LEVELS = 5
    ...
    def __penalties(self, p__, v__, x_g, res_multiplier):
        ...
        with tf.device('/CPU:0'):
            ...
            with ThreadPoolExecutor(max_workers=(100 * d_gu.shape[0])) as executor:
        ...
        with tf.device('/CPU:1'):
            ...
            with ThreadPoolExecutor(max_workers=(100 * d_ub.shape[0])) as executor:
    ...
    def __optimize_waiting_states(self, nu, s_wait, a_wait, l_wait_star, e_wait_star, t_wait_star):
        ...
        :param t_wait_star: The temporal costs (final optimality) collection (tensor) for waiting states and actions
                            ($t_{\nu}^{*}(s; v_{r}, \theta_{c}^{*})$)
        """
        with ThreadPoolExecutor(max_workers=(100 * s_wait.shape[0] * a_wait.shape[0])) as executor:
    ...
    def __optimize_comm_states(self, nu, s_comm, a_comm, l_comm_star, e_comm_star, t_comm_star):
        ...
        :param t_comm_star: The temporal costs (final optimality) collection (tensor) for comm states and actions
                            ($t_{\nu}^{*}(s; \hat{\mathbf{x}}_{U}, 0 or 1)$)
        """
        with ThreadPoolExecutor(max_workers=(100 * s_comm.shape[0] * a_comm.shape[0])) as executor:
    ...
    def __value_iteration(self, dual_variable):
        ...
        max_workers_o = 200 * ((s_wait_size * a_wait_size) + (s_comm_size * a_comm_size))
        max_workers_a = 200 * (s_wait_size + s_comm_size)
...
if __name__ == '__main__':
    ...
    max_workers = 100 * avg_powers.shape[0] * packet_lens.shape[0]
